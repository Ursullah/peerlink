        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm">
                    ...
                    <p class="mt-2 text-2xl font-bold text-gray-900 dark:text-gray-100">KES
                        {{ number_format($stats['total_invested'] / 100, 2) }}</p>
                </div>
                <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm">
                    <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">Total Returned</h3>
                    <p class="mt-2 text-2xl font-bold text-green-600 dark:text-green-400">KES
                        {{ number_format($stats['total_returned'] / 100, 2) }}</p>
                </div>
                <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm">
                    <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">Profit Earned</h3>
                    <p class="mt-2 text-2xl font-bold text-green-600 dark:text-green-400">KES
                        {{ number_format($stats['profit_earned'] / 100, 2) }}</p>
                </div>
                <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm">
                    ...
                    <p class="mt-2 text-2xl font-bold text-blue-600 dark:text-blue-400">
                        {{ $stats['active_investments'] }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        Pending Interest: KES {{ number_format($stats['pending_interest'] / 100, 2) }}
                    </p>
                    <a href="{{ route('lender.loans.investments') }}"
                        class="text-sm text-indigo-600 hover:underline">View All &rarr;</a>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm">
                    <h3 class="text-lg font-semibold mb-4 text-gray-900 dark:text-gray-100">Investment Status Breakdown
                    </h3>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                if (ctxPie && pieData && pieData.labels.length > 0) {
                    ...
                    const backgroundColors = pieData.labels.map(label => pieColors[label] ||
                    '#9CA3AF'); // Use gray as fallback
                        ...
                        data: {
                            ...
                            datasets: [{
                                ...
                                borderColor: document.documentElement.classList.contains('dark') ?
                                    '#4B5563' : '#FFFFFF', // gray-600 or white
                                borderWidth: 1
                            }]
                        },
                        options: {
                    ...
                    ctxPie.fillText("No investment data available yet.", ctxPie.canvas.width / 2, ctxPie.canvas.height /
                        2);
                }
            });
        </script>
